...
const submit = () => {
    form.post(route('testimoni.store')); // Menggunakan route untuk membuat URL
};
                ...
                <div class="bg-primary-100 border border-primary-300 shadow-sm rounded-lg min-w-[1000px]">
                    <div class="w-full text-gray-700 text-xl font-semibold p-5">
                        <form @submit.prevent="submit">
                            <div class="mt-4">
                                <label for="image" class="block font-medium text-sm text-gray-700">Gambar Testimoni</label>
                                <input type="file" id="image" name="image" class="mt-1 block w-full" accept="image/*" @change="handleImageChange">
